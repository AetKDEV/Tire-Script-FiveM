Citizen.CreateThread(function()
	while true do
		local allowedWeapons = {"WEAPON_KNIFE", "WEAPON_BOTTLE", "WEAPON_DAGGER", "WEAPON_HATCHET", "WEAPON_MACHETE", "WEAPON_SWITCHBLADE"} ---- Armes utilisable pour crevé
		local player = PlayerId()
		local plyPed = GetPlayerPed(player)
		local vehicle = GetClosestVehicleToPlayer()
		local animDict = "melee@knife@streamed_core_fps" ---- Animation couteau
		local animName = "ground_attack_on_spot" ---- Animation
		if vehicle ~= 0 then
			if CanUseWeapon(allowedWeapons) then
				...
				if closestTire ~= nil then
					...
					if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
						Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, tostring("~w~[E] crevé le pneu")) --------Texte sur les pneu
						if IsControlJustPressed(1, 38) then ------- [E]
							...
							local animDuration = GetAnimDuration(animDict, animName) ----durée de l'Animation
							TaskPlayAnim(plyPed, animDict, animName, 8.0, -8.0, animDuration, 15, 1.0, 0, 0, 0) ----durée de l'Animation
							Citizen.Wait((animDuration / 2) * 1000) ----durée de l'Animation
							...
							if driverServer == 0 then
								SetVehicleTyreBurst(vehicle, closestTire.tireIndex, 0, 100.0) ----pneu crever
							else
...
AddEventHandler("SlashTires:SlashClientTire", function(tireIndex)
	TriggerEvent("chatMessage", "^1Un joueur essaie de crever votre pneu")------- message
	local player = PlayerId()
	local plyPed = GetPlayerPed(player)
...
function GetClosestVehicleTire(vehicle)
	local tireBones = {"wheel_lf", "wheel_rf", "wheel_lm1", "wheel_rm1", "wheel_lm2", "wheel_rm2", "wheel_lm3", "wheel_rm3", "wheel_lr", "wheel_rr"} ------ Roues avant arrière ect...
	local tireIndex = {
		["wheel_lf"] = 0,
		["wheel_rf"] = 1,
		["wheel_lm1"] = 2,
...
function Draw3DText(x, y, z, text)   ----- Texte function
